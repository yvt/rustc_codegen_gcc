...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    ...
    fn saturating_add(&mut self, lhs: RValue<'gcc>, rhs: RValue<'gcc>, signed: bool, width: u64) -> RValue<'gcc> {
        ...
        if signed || !supports_native_type {
            ...
            let overflow =
                if supports_native_type {
                    let func_name =
                        match (width, signed) {
                            (8, true) => "__builtin_add_overflow",
                            (16, true) => "__builtin_add_overflow",
                            (32, true) => "__builtin_sadd_overflow",
                            (64, true) => "__builtin_saddll_overflow",
                            (128, true) => "__builtin_add_overflow",
                            _ => unreachable!(),
                        };
                    ...
                    let func_name =
                        match (width, signed) {
                            (128, true) => "__rust_i128_addo",
                            (128, false) => "__rust_u128_addo",
                            _ => unreachable!(),
                        };
            ...
            if signed {
                ...
                let max = self.gcc_not(self.gcc_int(result_type, 0));
                then_block.add_assignment(None, res, max);
                then_block.end_with_jump(None, after_block);
            }
        ...
        if signed || !supports_native_type {
            ...
            let overflow =
                if supports_native_type {
                    let func_name =
                        match (width, signed) {
                            (8, true) => "__builtin_sub_overflow",
                            (16, true) => "__builtin_sub_overflow",
                            (32, true) => "__builtin_ssub_overflow",
                            (64, true) => "__builtin_ssubll_overflow",
                            (128, true) => "__builtin_sub_overflow",
                            _ => unreachable!(),
                        };
                    let overflow_func = self.context.get_builtin_function(func_name);
                    self.overflow_call(overflow_func, &[lhs, rhs, res.get_address(None)], None)
                    ...
                    let func_name =
                        match (width, signed) {
                            (128, true) => "__rust_i128_subo",
                            (128, false) => "__rust_u128_subo",
                            _ => unreachable!(),
                        };
                    let param_a = self.context.new_parameter(None, result_type, "a");
                    let param_b = self.context.new_parameter(None, result_type, "b");
            ...
            if signed {
                ...
                then_block.add_assignment(None, res, self.gcc_int(result_type, 0));
                then_block.end_with_jump(None, after_block);
            }
